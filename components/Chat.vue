<template>
    <section class="flex flex-col min-w-[520px] w-full">
          ...
          >
            ...
            <div v-for="message in messages" class="flex items-end gap-3" :class="{'flex-row-reverse': !message.isBot}">
              <!-- Profile Image -->
              ...
              >
                <!-- Message Content -->
                <div v-if="message.isBot" v-html="message.message">
                </div>

                <div v-else class="break-all text-pretty">
                 {{message.message}}
                </div>
                <!-- Time -->
